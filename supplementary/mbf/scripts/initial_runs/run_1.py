...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.low_pass_filter = nn.Conv1d(hs, hs, kernel_size=3, padding=0, bias=False, groups=hs)
        self.band_pass_filter = nn.Conv1d(hs, hs, kernel_size=3, padding=0, bias=False, groups=hs)
        self.high_pass_filter = nn.Conv1d(hs, hs, kernel_size=3, padding=0, bias=False, groups=hs)

        # Initialize filter weights and make them non-trainable
        low_pass_weights = torch.tensor([1/3, 1/3, 1/3], dtype=torch.float).view(1, 1, 3).repeat(hs, 1, 1)
        self.low_pass_filter.weight = nn.Parameter(low_pass_weights, requires_grad=False)

        band_pass_weights = torch.tensor([0.5, 0, -0.5], dtype=torch.float).view(1, 1, 3).repeat(hs, 1, 1)
        self.band_pass_filter.weight = nn.Parameter(band_pass_weights, requires_grad=False)

        high_pass_weights = torch.tensor([-1.0, 2.0, -1.0], dtype=torch.float).view(1, 1, 3).repeat(hs, 1, 1)
        self.high_pass_filter.weight = nn.Parameter(high_pass_weights, requires_grad=False)

        # Learnable gates for fusing filter outputs, per head
        self.fusion_gates = nn.Parameter(torch.ones(1, self.n_head, 1, 1, 3))
    ...
    def forward(self, x):
        ...
        y_conv_in = y.reshape(B * nh, T, hs).transpose(1, 2).contiguous()

        # Apply causal padding for Conv1d with kernel_size=3
        padding_size = self.low_pass_filter.kernel_size[0] - 1
        y_conv_in_padded = F.pad(y_conv_in, (padding_size, 0))

        # Apply filters
        y_low = self.low_pass_filter(y_conv_in_padded)
        y_band = self.band_pass_filter(y_conv_in_padded)
        y_high = self.high_pass_filter(y_conv_in_padded)

        # Reshape back: (B*nh, hs, T) -> (B*nh, T, hs) -> (B, nh, T, hs)
        y_low = y_low.transpose(1, 2).view(B, nh, T, hs)
        y_band = y_band.transpose(1, 2).view(B, nh, T, hs)
        y_high = y_high.transpose(1, 2).view(B, nh, T, hs)

        # Stack filtered outputs for gating
        y_filtered = torch.stack([y_low, y_band, y_high], dim=-1)  # (B, nh, T, hs, 3)
        ...
        # Apply per-head output gating
        y = y_fused * torch.sigmoid(self.head_gate)
