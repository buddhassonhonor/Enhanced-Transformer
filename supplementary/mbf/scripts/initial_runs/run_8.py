...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Dynamic, input-dependent gates for fusing filter outputs
        self.fusion_gate_proj = nn.Linear(hs, self.num_filters)
    ...
    def forward(self, x):
        ...
        # Apply dynamic, input-dependent fusion gates
        fusion_gates_raw = self.fusion_gate_proj(y_attn_output) # (B, nh, T, num_filters)
        fusion_gates = F.softmax(fusion_gates_raw, dim=-1).unsqueeze(-2) # (B, nh, T, 1, num_filters)
